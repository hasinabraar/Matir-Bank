...
const SamitySection = ({ samityData, userId, onMessage }) => {
  ...
  const submitGroup = async (e) => {
    ...
    setError('');
    try {
      const res = await samityAPI.createGroup(groupForm);
      if (res.data.success) {
        onMessage('success', 'Samity created');
        setGroupForm({ GroupName: '', LeaderID: userId });
      } else {
        setError(res.data.message || 'Failed to create samity');
      }
    } catch (err) {
      setError(err.response?.data?.message || 'Failed to create samity');
    }
  };
      ...
      {showForm && (
        <div style={{ marginBottom: '20px', padding: '20px', background: '#f8f9fa', borderRadius: '5px' }}>
          ...
          <form onSubmit={submitGroup}>
            ...
            <button type="submit" className="btn btn-primary">Create Samity</button>
          </form>
